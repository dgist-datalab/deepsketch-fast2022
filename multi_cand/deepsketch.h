...
class ANN {
	...
	std::vector<int> request(MYHASH h);
	void insert(MYHASH h, int label);
};

std::vector<int> ANN::request(MYHASH h) {
	vector<std::pair<int, int>> temp;

	// scan list
	for (int i = linear.size() - 1; i >= 0; --i) {
		temp.push_back({(linear[i] ^ h).count(), -hashtable[linear[i]].back()});
	}
	...
	for (int i = 0; i < objects.size(); ++i) {
		...
		temp.push_back({nowdist, -hashtable[now].back()});
	}
