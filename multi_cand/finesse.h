...
class Finesse {
	...
	std::vector<int> request(unsigned char* ptr);
	void insert(int label);
};

std::vector<int> Finesse::request(unsigned char* ptr) {
	...
	int cnt[SF_NUM] = {};
	for (int i = 0; i < SF_NUM; ++i) {
		if (sfTable[i].count(superfeature[i])) {
			cnt[i] = sfTable[i][superfeature[i]].size();
		}
	}
	...
	while (1) {
		...
		uint32_t r = full_uint32_t(gen2) % exist;
		for (int i = 0; i < SF_NUM; ++i) {
			if (cnt[i] != 0) {
				if (r == 0) {
					int elem = sfTable[i][superfeature[i]][cnt[i] - 1];
					if (!st.count(elem)) {
						st.insert(elem);
						ret.push_back(elem);
					}
